...
export default function AnalisePage() {
  ...
  const handleAnalisar = async () => {
    ...
    try {
      ...
      if (dir === "CALL" || dir === "PUT") {
        const nowSec = Math.floor(Date.now() / 1000);
        const targetTsSec = Math.floor(nowSec / 60) * 60 + 60;
        setTrade({
      ...
      setError(isNetwork ? `${msg}. Verifique se a API está rodando.` : msg);
    } finally {
      ...
      <header className="flex items-center justify-between px-6 py-4 border-b border-white/5 bg-white/[0.02] backdrop-blur-sm">
        <Link href="/" className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-xs text-[#8b9bb3]">Análise / Sinais</p>
          </div>
        </Link>
        <Link href="/probabilisticas" className="px-4 py-2 rounded-xl text-sm font-medium bg-[#3b82f6] hover:bg-[#2563eb] text-white transition-colors">
          ...
          <section className="rounded-2xl p-5 border border-white/10 bg-white/[0.04] backdrop-blur-sm shadow-xl shadow-black/20">
            <div className="flex flex-wrap gap-3 justify-between items-center">
              ...
              <button
                ...
                className="bg-gradient-to-r from-[#3b82f6] to-[#2563eb] hover:from-[#60a5fa] hover:to-[#3b82f6] disabled:from-[#1f2937] disabled:to-[#1f2937] disabled:cursor-not-allowed text-white font-semibold px-6 py-3 rounded-2xl text-sm flex items-center gap-2 transition-all duration-200"
              >
                {loadingSignal ? (
                  <>
            ...
            <div className="flex flex-wrap items-center gap-6 mt-4 pt-4 border-t border-white/5">
              ...
              <p className="text-xs text-[#8b9bb3]">Clique em Analisar para gerar o sinal para a próxima vela.</p>
            </div>
          </section>
                    ...
                    {trade.phase === "ARMED" && (
                      <>
                        <span className="text-[#8b9bb3]">
                          Sinal <span className={trade.direction === "CALL" ? "text-[#22c55e] font-semibold" : "text-[#ef4444] font-semibold"}>{trade.direction}</span> armado para vela que inicia em <span className="font-mono text-[#e6edf3]">{tsToTime(trade.targetTsSec)}</span>.
                        </span>
                        <span className="font-mono text-[#e6edf3]">
                          Entrada em {String(Math.floor(countdownSeconds / 60)).padStart(2, "0")}:{String(countdownSeconds % 60).padStart(2, "0")}
                        </span>
                      </>
                    )}
                    {trade.phase === "IN_TRADE" && (
                      <>
                        <span className="text-[#8b9bb3]">Operando vela iniciada em <span className="font-mono text-[#e6edf3]">{tsToTime(trade.targetTsSec)}</span>. Aguardando fechamento.</span>
                        <span className="font-mono text-[#e6edf3]">Fim em {String(Math.floor(countdownSeconds / 60)).padStart(2, "0")}:{String(countdownSeconds % 60).padStart(2, "0")}</span>
                      </>
                    )}
                    {trade.phase === "RESOLVED" && (
                      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-2 w-full">
                        <span className="text-[#8b9bb3]">
                          Resultado: <span className={`font-semibold ${trade.result === "WIN" ? "text-[#22c55e]" : trade.result === "LOSS" ? "text-[#ef4444]" : "text-[#8b9bb3]"}`}>{trade.result}</span>.
                        </span>
                        <span className="text-[#8b9bb3]">
                          Vela: <span className="font-mono text-[#e6edf3]">{tsToTime(trade.targetTsSec)}</span>
                          {trade.candleOpen != null && trade.candleClose != null && (
                            <> · <span className="font-mono text-[#e6edf3]">{trade.candleOpen.toFixed(5)} → {trade.candleClose.toFixed(5)}</span></>
                          )}
                        </span>
                      </div>
                    )}
          ...
          <section className="rounded-[18px] p-4 border border-white/10 bg-white/[0.03] backdrop-blur-sm shadow-xl shadow-black/20">
            ...
            {showCandles && (
              <div className="mt-3">
                ...
                >
                  {loadingCandles ? (
                    <><span className="inline-block w-4 h-4 border-2 border-[#3b82f6]/30 border-t-[#3b82f6] rounded-full animate-spin" /> Carregando…</>
                  ) : (
                    <>Atualizar Mercado <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M4 4v5h.582m15.356 2A8.001 8.001 0 004.582 9m0 0H9m11 11v-5h-.581m0 0a8.003 8.003 0 01-15.357-2m15.357 2H15" /></svg></>
                  )}
                </button>
                {error && candles.length === 0 ? (
                  <div className="py-4 px-3 rounded-xl bg-red-500/10 border border-red-500/30 text-red-200 text-sm flex flex-col gap-2">
                    ...
                    <button type="button" onClick={loadCandles} className="px-4 py-2 rounded-xl text-sm font-medium bg-red-500/20 hover:bg-red-500/30 text-white transition-colors">Tentar novamente</button>
                  </div>
                ) : candles.length === 0 ? (
                        ...
                        {[...candles].reverse().map((c) => (
                          <tr key={c.ts} className="border-b border-white/5 hover:bg-white/[0.03] transition-colors">
                            ...
                            <td className="text-center py-2.5">
                              {c.color === "green" ? <span className="inline-block w-2 h-2 rounded-full bg-[#22c55e]" title="Alta" /> : c.color === "red" ? <span className="inline-block w-2 h-2 rounded-full bg-[#ef4444]" title="Baixa" /> : <span className="inline-block w-2 h-2 rounded-full bg-[#8b9bb3]" title="Doji" />}
                            </td>
                          </tr>
                        ))}
                      </tbody>
